...
static void TextureIteration(Texture *texture, size_t idx, void *user)
{
    ...
    if(igImageButton(texture->name, (ImTextureRef){ ._TexID = texture->texture1 }, size, (ImVec2){ 0, 0 }, (ImVec2){ 1, 1 }, (ImVec4){ 1, 1, 1, 1 }, (ImVec4){ 1, 1, 1, 1 }))
    {
    ...
    {
        igImage((ImTextureRef){ ._TexID = texture->texture1 }, size, (ImVec2){ 0, 0 }, (ImVec2){ 1, 1 });
...
void TexturesWindow(bool *p_open, EdState *state)
{
    if(igBegin("Texture Browser", p_open, ImGuiWindowFlags_MenuBar))
    {
        ...
        igInputTextWithHint("Filter", "Search", state->data.textureFilter, sizeof state->data.textureFilter, 0, NULL, NULL);
        igSameLine(0, 16);
